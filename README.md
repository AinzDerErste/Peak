...
| `SetIslandBanner(UIElement?)` | Attach a banner element below the island. Width tracks the IslandBorder's runtime `ActualWidth` so the strip stretches/shrinks with state transitions. Single-slot (last writer wins); pass `null` to clear. | Live-progress strip for a download (`PeakMediaDownload` plugin uses this); "joining call…" status while a Discord connection establishes; transient background-task acknowledgements that shouldn't obscure the island's own widgets. |
